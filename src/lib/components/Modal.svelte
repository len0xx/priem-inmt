<script lang="ts">
    ...
    const closeTheModal = () => hidden = true
...
<div class="modal-wrapper" class:hidden>
    <div class="shadow" on:click={closeTheModal}></div>
    <div class="modal-svelte align-{align}">
        { #if closable }
            <div class="close" on:click={closeTheModal}>
        ...
        { /if }
        <div class="modal-content-svelte">
            <slot />
        </div>
        <div class="footer">
            <slot name="footer" />
...
<style>
    ...
    :global(.modal-wrapper .modal-svelte > .close) {
        display: grid;
        place-content: center;
        place-items: center;
        position: absolute;
    ...
    :global(.modal-wrapper .modal-svelte > .close:hover) {
        background: rgba(0, 0, 0, 0.1);
    }

    .modal-wrapper .modal-svelte > .modal-content-svelte {
        padding: 4em;
    }
